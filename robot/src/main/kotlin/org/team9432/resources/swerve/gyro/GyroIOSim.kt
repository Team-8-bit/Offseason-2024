...
class GyroIOSim: GyroIO {
    ...
    override fun updateInputs(inputs: GyroIO.GyroIOInputs) {
        ...
        currentGyroDriftAmount = currentGyroDriftAmount.rotateBy(
            Rotation2d.fromRadians(MapleCommonMath.generateRandomNormal(0.0, currentTickDriftStdDevRad))
        )

        inputs.connected = gyroPhysicsSimulationResults.hasReading
        inputs.odometryYawPositions = gyroPhysicsSimulationResults.odometryYawPositions.map { robotFacing: Rotation2d? -> robotFacing!!.rotateBy(currentGyroDriftAmount) }.toTypedArray()
    ...
    class GyroPhysicsSimulationResults {
        ...
        val odometryYawPositions: Array<Rotation2d?> = arrayOfNulls<Rotation2d>(SIMULATION_TICKS_IN_1_PERIOD)

        init {
            Arrays.fill(odometryYawPositions, Rotation2d())
        }
    }
